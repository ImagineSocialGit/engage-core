...
| Reporting contract | Reporting foundation audit and module documentation | Complete | The durable privacy-first Reporting contract and phased Webinar-first plan are now defined. Next: shared no-op/contributor seams, Reporting persistence, ingestion/session/attribution foundation, setup validation, and Project State classification. |
...
The next Reporting branch is the **observation foundation**: shared Reporting contracts/registry, no-op behavior while Reporting is disabled, Reporting config and persistence, idempotent observation ingestion, ephemeral session resolution, attribution normalization, projection checkpoints, setup validation, explicit Project State classification, and focused privacy/boundary tests. Browser/Webinar instrumentation follows only after reconciling the fresh Webinar/frontend source.
