...
Tasks is a reusable capability module for tracked manual human actions and dependencies.

A Task is one generic live work record. Do not define separate mutually exclusive Task categories such as standalone, Contact-related, template-backed, or automation-created. Describe each Task across independent dimensions:
...
domain linkage
    unlinked or linked to one or more domain records
...
Repeated similar manual no-template Tasks are the primary Task-created signal for Automation Opportunity suggestions.
...
linked to several records in one module
linked to several records across modules
```
...
links:
    ...
    Appointment #482 -> result, once created
```

Tasks owns the generic link structure and generic role vocabulary. Linked modules own the domain meaning and presentation of their own records through Tasks-owned resolver/provider extension seams.

Tasks must not import Scheduling, Documents, PetServices, Mortgage, FlowRoutes, or other higher-level module models merely to interpret linked records.

Do not store canonical Task relationships as arbitrary IDs in `meta`.

Do not add module-specific foreign keys such as `pet_id`, `appointment_id`, `document_request_id`, or `mortgage_file_id` to `tasks`.
...
- task lifecycle actions/events;
- task assignment and responsibility semantics;
- task due dates and priority/default behavior;
- linked-record presentation contracts/registries and fallback behavior;
- task digest trigger/cadence behavior;
- Task index/show surfaces.

TaskTemplates are optional reusable definitions/defaults for manual Tasks and required for automation-created Tasks.
...
Tasks depends on Core and must remain fully useful with only Core enabled.

Core-only Task capability includes:

```text
ad hoc no-template manual Task creation
template-backed manual Task creation
unlinked Tasks
Contact-linked Tasks
dedicated Task index/show surfaces
Task lifecycle actions
TaskTemplate defaults
neutral Task lifecycle events
```

InternalNotifications and Messaging are optional capabilities for assignment notification and digest delivery. Core Task creation, lifecycle, templates, links, and UI must not require them.

InternalNotifications may contribute TeamMember assignment/recipient behavior through public Tasks-owned seams when enabled. Tasks core paths should not hard-require TeamMember models/tables.

FlowRoutes may consume Tasks through public task actions and neutral automation events. Tasks must not import FlowRoutes internals.

### Task UI

Task UI should answer immediately:

```text
WHY is this Task here?
WHAT needs to be done?
HOW can the user complete or advance it quickly?
```

Dedicated Task index and Task show surfaces are required.

The first implementation may be information-dense and function-first; final UI polish may follow later.

Task pages should present linked records through human-readable module-provided labels/details/URLs, grouped by generic role where useful.

Do not expose raw morph fields as primary UI labels.

### Assignment and responsibility

Task assignment and responsible-party semantics belong to Tasks.

Current responsibility concepts:

```text
assigned_to
    optional internal owner/tracker when a supported assignee provider exists

responsible_party
    who or what must actually perform the manual action

responsible
    optional concrete responsible model
```
...
Assignment and responsibility do not imply notification delivery, Messaging consent, login access, or direct task completion ability.

### Automation events and FlowRoutes boundary

Tasks may emit neutral lifecycle events such as:

```text
task.completed
```

A valid Task event may have `contact_id = null` and use the Task itself as the event subject.

FlowRoutes should listen to generic `AutomationEventRecorded`, not Task-specific producer events.

FlowRoutes owns route progress, plans, progress items, created-artifact references, correlation, and resume matching.
...
When FlowRoutes creates a Task, it should preserve the Task identity in FlowRoutes-owned state and use neutral Task event identity plus explicit correlation when later resume matching is required.
...
FlowRoutes owns route provenance and correlation. That does not mean every artifact-owning module should import FlowRoutes models or add FlowRoutes-specific foreign keys.

Preferred boundary:

```text
FlowRoutes
    owns route progress/plans/progress items
    stores created artifact type/id
    owns explicit correlation and resume matching

Owning module artifact
    owns its own business state and lifecycle
...
FlowRoutes create_task point
    ...
    -> FlowRoutes progress item stores created Task identity
    -> Task emits neutral lifecycle events
    -> FlowRoutes matches/resumes through FlowRoutes-owned correlation state
```

Tasks should not store `flow_route_*` foreign keys or import FlowRoutes-owned models merely to preserve provenance.

Future modules should follow the same ownership test before adding provenance columns. Prefer FlowRoutes-owned created-artifact references/correlation unless the artifact-owning module has an independently justified neutral provenance contract.

Good:

```text
FlowRoutes -> Tasks public action -> Task
FlowRoutes progress item records created Task type/id
Tasks emits AutomationEventRecorded(task.completed)
FlowRoutes resumes from its own correlation state
```
...
Every route-created artifact receives FlowRoutes foreign keys merely for schema symmetry.
```
