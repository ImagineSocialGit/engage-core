...
- [x] Phase 4B.2A: define the Scheduling-owned location/snapshot boundary and retain Location as a separate optional silent capability with its own transient normalization contract.
- [x] Phase 4B.2B1: add Scheduling-owned deterministic address normalization and canonical phone/virtual/fixed/customer-site commitment snapshots, persist authoritative BookingHold location snapshots, and copy held snapshots into Appointment conversion/rescheduling without creating Location rows.
- [x] Phase 4B.2B2: add closed CRM service-location authoring and customer-site public/CRM raw-address collection with server-owned normalization before commitment.
...
- [ ] Phase 4B.2D: add authoritative range-duration booking for consecutive-day stays such as pet boarding, including check-in/check-out intervals, capacity/resource occupancy across the complete stay, holds, cancellation/rescheduling, and lifecycle; fixed-location PetServices stays do not require Location, while PetServices owns pet association, vaccination/compliance, intake, feeding/medication, and other boarding-specific meaning.
