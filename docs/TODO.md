...
- [ ] Phase 4B.2C: add Scheduling-owned provider-neutral travel-time resolution, conservative fallback, adjacent-Appointment checks, reservation/direct-creation revalidation, and admin reschedule slot suggestions that preserve the source booking criteria; when optional Location is separately enabled, allow geographic/travel convenience to improve in-person suggestion ranking without making Location a Scheduling dependency.
