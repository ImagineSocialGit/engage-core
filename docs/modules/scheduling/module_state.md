...
Each generated channel copy also receives a stable Messaging `MessageTemplatePreset` and `MessageTemplateCatalogEntry`, grouped under **Scheduling → Appointment Communications** in the general Message Templates workspace. Those records are discovery/authoring identities only; timing remains owned by Scheduling and the immutable MessageChain. Editing appointment copy from the general Message Templates workspace publishes the canonical template version and, through the generic Messaging template-publication hook registry, republishes the current appointment MessageChain for future enrollments. Existing appointment enrollments remain pinned to their prior chain/template versions. Removing a step or channel from the Scheduling plan deactivates its catalog identity instead of leaving stale appointment copy visible in the library.
For environments that already had a 23A3D appointment plan before catalog registration existed, `php artisan scheduling:communications:sync-catalog` idempotently materializes the missing preset/catalog identities from the current published plan. It does not invent a plan when none is configured.
