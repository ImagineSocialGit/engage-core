...
- [ ] Enable Resend Open Tracking when this client should collect provider-reported email-open evidence.
- [ ] Leave Resend Click Tracking OFF.

Engage Core stores Resend opens as weak tracking-pixel evidence, not proof that a person read the message. Engage Core owns explicit CTA engagement tracking, so Resend Click Tracking adds another redirect layer and should not be enabled on top of Messaging `tracking_key` links.
...
The omitted events either belong to the inbound path or do not currently have an Engage Core runtime contract. `email.opened` belongs on this Messaging endpoint when Open Tracking is enabled; it is normalized into aggregate weak evidence rather than treated as a definitive human interaction.
...
[ ] Resend Open Tracking matches the client's email-open evidence policy; Click Tracking remains off
