...
- Commerce requests may use Engage Core for custom storefront/presentation, cross-channel inventory orchestration, provider-authoritative pricing/promotion display, and promotion/source attribution while external providers retain the configured authoritative roles for discount eligibility/calculation, payment processing, deep store operations, warehouse/shipping fulfillment, inventory, or other specialized facts/operations. A QR/promotion entry point may resolve through Core to provider-backed discount context, but do not invent a second Core discount engine. Do not assume one commerce provider or hard-code an example provider combination. Avoid adding middleware SaaS as an architectural requirement when direct provider packages can connect the client's existing systems. Location remains supporting geographic capability rather than a GIS, routing, or map-provider replacement.
