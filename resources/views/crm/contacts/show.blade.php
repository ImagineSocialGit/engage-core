...
>
    ...
    >
                ...
                <x-ui.card class="space-y-5" data-module-panel="inbound_messaging">
                    <div class="flex items-start justify-between gap-3">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-slate-500">
                                What they said, what we sent, and the fastest path to answer.
                            </p>
                        </div>
                            ...
                            <div class="flex flex-wrap items-center justify-between gap-2">
                                ...
                                @if($latestInboundReply['occurred_at'] ?? null)
                                    <span class="text-xs text-slate-500">
                                        {{ $latestInboundReply['occurred_at']->copy()->setTimezone($clientTimezone)->format('M j, g:i A') }}
                                    </span>
                                @endif
                            </div>
                            ...
                            <div class="rounded-2xl border border-slate-200 bg-white p-4" data-contact-conversation-composer>
                                <div class="flex flex-wrap items-center justify-between gap-2">
                                    <div>
                                        ...
                                        @if(filled($conversationReply['destination'] ?? null))
                                            <p class="mt-0.5 break-all text-xs text-slate-500">
                                                {{ $conversationReply['destination'] }}
                                            </p>
                                        @endif
                                    </div>
                                </div>

                                @if($conversationReply['can_send'] ?? false)
                                    <form
                                        method="POST"
                                        action="{{ route('crm.contacts.conversation.reply.store', [
                                            $contact,
                                    ...
                                    >
                                        ...
                                        <input
                                            type="hidden"
                                            name="reply_request_key"
                                            value="{{ old('reply_request_key', (string) \Illuminate\Support\Str::uuid()) }}"
                                        >

                                        @if(($conversationReply['channel'] ?? null) === 'email')
                                            <div>
                                                <label for="reply-subject" class="text-xs font-semibold text-slate-700">
                                                    Subject
                                                </label>
                                                <input
                                                    id="reply-subject"
                                                    ...
                                                    type="text"
                                                    maxlength="998"
                                                    value="{{ old('reply_subject', $conversationReply['subject'] ?? '') }}"
                                                    class="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"
                                                >
                                                @error('reply_subject')
                                        ...
                                        <div>
                                            <label for="reply-body" class="text-xs font-semibold text-slate-700">
                                                Message
                                            </label>
                                            <textarea
                                                id="reply-body"
                                                ...
                                                class="mt-1 w-full resize-y rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm leading-6 text-slate-900 outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"
                                        ...
                                        @error('reply_request_key')
                                            <p class="text-xs font-semibold text-red-600">{{ $message }}</p>
                                        @enderror

                                        <x-ui.button type="submit" class="w-full">
                                            Send {{ $conversationReply['channel_label'] ?? 'Reply' }}
                                        </x-ui.button>

                                        <p class="text-xs leading-5 text-slate-500">
                                            Uses the same Messaging permission, suppression, queue, and reply-correlation safeguards as other sends.
                                        </p>
                                    </form>
                                @else
                    ...
                    @else
                        <div class="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-5 text-sm text-slate-500">
                            No inbound replies yet. When someone replies, the message they answered will appear here with it.
                        </div>
                    @endif

                    @if($conversationTimeline->isNotEmpty())
                        <div class="max-h-[34rem] overflow-y-auto rounded-2xl border border-slate-200 bg-white">
                                        ...
                                        <div class="flex flex-wrap items-start justify-between gap-3">
                                            <div class="min-w-0">
                                                <div class="flex flex-wrap items-center gap-2">
                                                    ...
                                                    @endif
                                                    @if(($conversationItem['direction'] ?? null) === 'outbound' && filled($conversationItem['status'] ?? null))
                                                        <span class="text-[11px] font-semibold text-slate-400">
                                                            {{ str($conversationItem['status'])->replace('_', ' ')->title() }}
                                                        </span>
                                                    @endif
                                                </div>
                                            ...
                                            @if($conversationItem['occurred_at'] ?? null)
                                                <span class="shrink-0 text-xs text-slate-500">
                                                    {{ $conversationItem['occurred_at']->copy()->setTimezone($clientTimezone)->format('M j, g:i A') }}
                                                </span>
                                            @endif
                                        </div>
                                    </div>
        ...
        <div id="contact-activity" class="grid gap-6 xl:col-start-1">
        ...
        @if(module_enabled('messaging'))
            <x-ui.card class="{{ $messagesPanelClass }} xl:col-start-1" data-module-panel="messaging">
                <div>
                    <h3 class="text-lg font-semibold tracking-tight">
                        Messages & consent
                    </h3>
