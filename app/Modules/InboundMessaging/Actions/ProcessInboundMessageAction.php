...
class ProcessInboundMessageAction
{
    ...
    private function process(int $receiptId, int $inboundMessageId): ?string
    {
        $receipt = InboundMessageReceipt::query()->lockForUpdate()->findOrFail($receiptId);

        if ($receipt->status === InboundMessageReceipt::STATUS_COMPLETED) {
            return $receipt->response_message;
        }

        $inboundMessage = InboundMessage::query()->lockForUpdate()->findOrFail($inboundMessageId);
    ...
    private function markRetryableFailure(int $receiptId, Throwable $exception): void
    {
        DB::transaction(function () use ($receiptId, $exception): void {
            $receipt = InboundMessageReceipt::query()->lockForUpdate()->find($receiptId);
