...
class ProcessHighwayReadService
{
    ...
    public function read(): array
    {
        ...
        $routeIds = $routes->pluck('id')->map(fn (mixed $id): int => (int) $id)->all();
        ...
        $processes = $routes
            ->map(function (object $route) use ($points, $statusNames): array {
                ...
                return [
                    ...
                    'category_label' => $this->categoryLabel($definitionMeta['category'] ?? null),
                    ...
                    'edit_url' => $this->flowRoutesEnabled()
                        ? route('crm.flow-routes.show', ['flowRoute' => (int) $route->id])
                        : null,
                ];
            })
            ->values();
        ...
        $groups = $processes
            ...
            ->map(function (Collection $items, string $key): array {
                return [
                    'key' => $key,
                    'label' => (string) ($items->first()['category_label'] ?? Str::headline($key)),
                    'processes' => $items->values(),
                ];
            })
            ->sortBy(fn (array $group): int => $this->categoryPriority($group['key']))
            ->values();
    ...
    private function flowRoutesAvailable(): bool
    {
        return $this->flowRoutesEnabled()
            && Schema::hasTable('flow_routes')
            && Schema::hasTable('flow_route_points');
    }

    private function flowRoutesEnabled(): bool
    {
    ...
    private function triggerSummary(object $route, array $meta, array $statusNames): string
    {
        ...
        if ($triggerType === 'contact_status') {
            ...
            $fromKeys = is_array($transition)
                ? array_values(array_filter($transition['from_contact_status_keys'] ?? [], 'is_string'))
                : [];
     ...
     *  @param array<string, string> $statusNames
     */
    private function pointDetail(string $type, array $definition, array $statusNames): ?string
    {
        return match ($type) {
            ...
            'cancel_campaign_family', 'pause_campaign_family' => $this->detail('Nurture family', $definition['family_key'] ?? null),
            'create_task' => $this->detail('Task', $definition['task_template_key'] ?? null),
            'add_contact_tag', 'remove_contact_tag' => $this->detail('Tag', $definition['tag'] ?? null),
            'change_status' => isset($definition['contact_status_key']) && is_string($definition['contact_status_key'])
    ...
    private function outcomes(Collection $points, array $statusNames): array
    {
        ...
        return $points
            ->filter(fn (object $point): bool => in_array((string) $point->type, $outcomeTypes, true))
    ...
    private function categoryLabel(mixed $category): string
    {
        return match ($this->categoryKey($category)) {
            ...
            default => Str::headline($this->categoryKey($category)),
        };
    }
