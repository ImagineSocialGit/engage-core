...
# Produces: file_dumps/ProjectState_dependency_cone_dump.txt
#
# Project State is shared application infrastructure rather than an app/Modules
# module, so this script uses explicit Project State roots and then resolves:
#   - project-local PHP class dependencies
#   - config(...) dependencies
#   - configured Project State table ownership
#   - migrations that create, alter, rename, or drop those tables
#   - Eloquent model table ownership for recursively resolved runtime classes

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd -- "$SCRIPT_DIR/.." && pwd)"
OUTPUT_DIR="$ROOT_DIR/file_dumps"
OUTPUT_FILE="$OUTPUT_DIR/ProjectState_dependency_cone_dump.txt"
...
if [[ ! -d "$ROOT_DIR/app/Support/ProjectState" ]]; then
    echo "Error: app/Support/ProjectState was not found under: $ROOT_DIR" >&2
    exit 1
fi

if [[ ! -f "$ROOT_DIR/config/project_state.php" ]]; then
    echo "Error: config/project_state.php was not found under: $ROOT_DIR" >&2
    exit 1
fi
...
PROJECT_STATE_TABLES="$TMP_DIR/project-state-tables.txt"
...
# First-class Project State roots.
SEED_PATHS=(
    "app/Http/Controllers/CRM/ProjectStateController.php"
    "routes/crm.php"
    "config/project_state.php"
    "tests/Feature/Clients/ProjectStateClientEnvironmentContractTest.php"
...
SEED_DIRECTORIES=(
    ...
    "resources/views/crm/project-state"
    "config/project_state"
    "tests/Feature/ProjectState"
)

for relative_path in "${SEED_PATHS[@]}"; do
    add_file "$ROOT_DIR/$relative_path" "$SEED_FILES"
done

for relative_directory in "${SEED_DIRECTORIES[@]}"; do
    add_directory "$ROOT_DIR/$relative_directory" "$SEED_FILES"
done

# Include every repository file that explicitly references the Project State
# infrastructure, route surface, environment key, or documentation names.
SEARCH_ROOTS=(
    "app"
    "bootstrap"
    "config"
...
# Read the complete configured Project State contract and policy registry. This
# catches table ownership that cannot be discovered from DB::table() calls.
resolve_project_state_tables() {
    php -d display_errors=1 -r '
require $argv[1]."/vendor/autoload.php";
$config = require $argv[1]."/config/project_state.php";
$tables = [];

foreach (($config["sections"] ?? []) as $section) {
    foreach (($section["tables"] ?? []) as $table => $_definition) {
...
foreach (($config["table_policies"] ?? []) as $table => $_policy) {
    if (is_string($table) && trim($table) !== "") {
        $tables[$table] = true;
    }
}
...
' "$ROOT_DIR" | sort -u > "$PROJECT_STATE_TABLES"

    while IFS= read -r table; do
        add_migrations_for_table "$table"
    done < "$PROJECT_STATE_TABLES"
}
...
resolve_non_class_dependencies() {
    ...
    while IFS= read -r config_root; do
        ...
        if [[ "$config_root" == "project_state" ]]; then
            add_file "$ROOT_DIR/config/project_state.php" "$CONFIG_FILES"
            add_directory "$ROOT_DIR/config/project_state" "$CONFIG_FILES"
            continue
        fi
...
# Traverse Project State implementation and tests. routes/crm.php and general
# documentation remain evidence-only so unrelated route imports do not pull the
# entire application into the dump.
{
    find "$ROOT_DIR/app/Support/ProjectState" -type f -name '*.php' -print
    [[ -f "$ROOT_DIR/app/Http/Controllers/CRM/ProjectStateController.php" ]] \
        && printf '%s\n' "$ROOT_DIR/app/Http/Controllers/CRM/ProjectStateController.php"
    [[ -f "$ROOT_DIR/tests/TestCase.php" ]] \
        && printf '%s\n' "$ROOT_DIR/tests/TestCase.php"
    [[ -d "$ROOT_DIR/tests/Feature/ProjectState" ]] \
        && find "$ROOT_DIR/tests/Feature/ProjectState" -type f -name '*.php' -print
    [[ -f "$ROOT_DIR/tests/Feature/Clients/ProjectStateClientEnvironmentContractTest.php" ]] \
        && printf '%s\n' "$ROOT_DIR/tests/Feature/Clients/ProjectStateClientEnvironmentContractTest.php"
} | sort -u > "$QUEUE_FILES"
...
# Always include the complete Project State config tree and any migration whose
# filename explicitly names project_state, even if table-index parsing is unable
# to associate it with a configured table.
add_file "$ROOT_DIR/config/project_state.php" "$CONFIG_FILES"
add_directory "$ROOT_DIR/config/project_state" "$CONFIG_FILES"

while IFS= read -r migration; do
    add_file "$migration" "$MIGRATION_FILES"
done < <(find "$ROOT_DIR/database/migrations" -maxdepth 1 -type f -iname '*project_state*.php' -print 2>/dev/null || true)
...
PROJECT_STATE_TABLE_COUNT="$(sort -u "$PROJECT_STATE_TABLES" | wc -l | tr -d ' ')"
MODEL_TABLE_COUNT="$(sort -u "$MODEL_TABLES" | wc -l | tr -d ' ')"
MIGRATION_DEPENDENCY_COUNT="$(sort -u "$MIGRATION_FILES" | wc -l | tr -d ' ')"
GENERATED_AT="$(date -u '+%Y-%m-%dT%H:%M:%SZ')"

{
    echo "Engage Core Project State Dependency Cone Dump"
    echo "==============================================="
    echo
    echo "Surface: Project State shared infrastructure"
    echo "Generated: $GENERATED_AT"
    echo "Repository root: $ROOT_DIR"
    echo "Included files: $FILE_COUNT"
    echo "First-class Project State seed files: $SEED_COUNT"
    echo "Explicit Project State reference matches: $MATCHED_COUNT"
    ...
    echo "Configured and policy-controlled tables discovered: $PROJECT_STATE_TABLE_COUNT"
    ...
    echo "  - complete app/Support/ProjectState tree"
    echo "  - ProjectStateController, CRM route evidence, and Project State Blade surface"
    echo "  - complete config/project_state.php and config/project_state tree"
    echo "  - complete tests/Feature/ProjectState tree and client environment contract test"
    echo "  - Project State extension guide and transfer runbook"
    echo "  - repository-wide explicit Project State references"
    echo "  - recursive project-local PHP import and same-namespace class resolution"
    echo "  - runtime config(...) root resolution"
    echo "  - configured section and table-policy ownership resolution"
    echo "  - explicit runtime DB table resolution"
    echo "  - runtime Eloquent model getTable() resolution"
    echo "  - migrations that create, alter, rename, or drop discovered tables"
    echo "  - routes/crm.php retained as evidence-only to avoid unrelated route imports"
    echo "  - all environment files excluded except .env.example"
    echo
    echo "PROJECT STATE TABLES"
    echo "===================="
...
echo "First-class Project State seeds: $SEED_COUNT"
echo "Explicit Project State matches: $MATCHED_COUNT"
...
echo "Configured/policy-controlled tables: $PROJECT_STATE_TABLE_COUNT"
echo "Runtime Eloquent model tables resolved: $MODEL_TABLE_COUNT"
